...
const Sidebar: React.FC<SidebarProps> = ({ isSidebarOpen }) => {
  const [sidebarItems, setSidebarItems] = useState<SidebarItemNode[]>([]);
  ...
  useEffect(() => {
    sidebarService
      ...
      .then((items) => {
        ...
        // Initialize openSubmenus based on the number of items
        const initialOpenSubmenus = Array(items.length).fill(false);
        ...
        items.forEach((module, moduleIndex) => {
          module.children?.forEach((service, serviceIndex) => {
            ...
            if (location.pathname === fullPath) {
              ...
              // Open the matched submenu
              initialOpenSubmenus[moduleIndex] = true;
            }
          });
        });
